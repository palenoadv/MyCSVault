...
def pick(paragraphs, select, k):
    ...
    for s in paragraphs:
        if select(s):
            result.append(s)
    if k < len(result): return result[k]
    else: return ""
...
def about(keywords):
    ...
    def insert_sen(sentence):
        for s in sentence.split(sep=None, maxsplit=-1):
            if lower(remove_punctuation(s)) in keywords:
                return True
        return False
    return insert_sen
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    result = typed_word
    for s in word_list:
        diff_new = diff_function(typed_word, s, limit)
        if diff_new < diff and diff_new <= limit:
            diff = diff_new
            result = s
    return result
...
def fastest_words(words_and_times):
    ...
    for i in word_indices:
        best_time = max([lst[i] for lst in times]) + 1
        best_player = 0
        for j in player_indices:
            if times[j][i] < best_time:
                best_time = times[j][i]
                best_player = j
        result[best_player].append(words[i])
    return result
